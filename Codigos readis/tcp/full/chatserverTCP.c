...
void chat(int sd2){
	...
	while(served == 0){
		...
		if(!strcmp(inbuf, "SALIR")){
			sprintf(outbuf, "SALIR");
...
void *manage_connection(void *sdp) {
	...
	printf("-(IN THREAD)- enviar a contactos en linea\n");

	printf("-(IN THREAD)- simulacion de chat\n");
...
int main(int argc, char** argv){
	...
	if(argc < 2){
		printf("\nUsage: ./servertcp 'IP' ['puerto'] \nUse 127.0.0.1 para localhost y si quiere un purto\n\n");
		return;
	}
